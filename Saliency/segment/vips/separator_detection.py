...
from .seperator_rule import SeparatorRule
...
class SeparatorDetection:
    ...
    def service(self, blocks, separatorType):
        ...
        self.threeStep()
        # #print(
        #     str(self.separatorType)
        #     + "-SeparatorVo.list.size::"
        #     + str(len(self.separatorList))
        # )
        ...
        for block in blocks:
            ...
            for sep in temp:
                ...
                if SeparatorRule.horizontalRule1(block, sep):
                    ...
                    newSep = SeparatorVo(
                        0, y, self.width, (sep.y + sep.height) - y, self.separatorType
                    )
                    if newSep.height != 0:
    ...
    def verticalDetection(self, blocks):
        ...
        for b1 in blocks:
            ...
            if leftMinW < sys.maxsize:
                separator = SeparatorVo(
                    leftX, leftY, leftW, leftH, SeparatorVo.TYPE_VERTICAL
                )
                self.separatorList.append(separator)
            if rightMinW < sys.maxsize:
                separator = SeparatorVo(
                    rightX, rightY, rightW, rightH, SeparatorVo.TYPE_VERTICAL
                )
                self.separatorList.append(separator)
