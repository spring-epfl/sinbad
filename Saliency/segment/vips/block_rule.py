class BlockRule:
    ...
    def dividable(block):
        ...
        if name in BlockRule.ATOMIC_ELEMENTS:
            return False
    ...
    def rule9(block):
        ...
        if count > 0:
            if (
                node.visual_cues["bounds"]["x"] * node.visual_cues["bounds"]["y"]
                < BlockRule.threshold
            ):
                ret = False
    ...
    def isValidNode(node):
        ...
        is_visible = node.visual_cues["is_visible"]
        has_hidden_class = any(
            ["hidden" in v for k, v in node.attributes if k == "class"]
        )
    ...
    @staticmethod
    def isTextNode(node):
        return node.nodeType == 3 or node.nodeName == "IMG" or node.nodeName == "IFRAME"
